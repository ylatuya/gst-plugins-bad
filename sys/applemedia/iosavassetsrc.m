...
static GstStaticPadTemplate audio_factory = GST_STATIC_PAD_TEMPLATE ("audio",
    ...
    GST_PAD_SOMETIMES,
    GST_STATIC_CAPS (
        "audio/x-raw-int, width = (int) 24, depth = (int) 24, " COMMON_CAPS)
    );
...
GST_BOILERPLATE (GstAVAssetSrc, gst_avasset_src, GstElement,
    GST_TYPE_ELEMENT);
...
- (id) initWithURI:(gchar*)uri : (GError **)error;
{
  ...
  *error = NULL;
  ...
  audio_caps = gst_caps_from_string ("audio/x-raw-int, "
...
- (GstCaps *) getCaps: (GstAVAssetReaderMediaType) type
{
  ...
  if (type == GST_AVASSET_READER_MEDIA_TYPE_AUDIO) {
    ...
    caps = gst_caps_new_simple ("video/x-raw-yuv",
        ...
        "height", G_TYPE_INT, (int) track.naturalSize.height, NULL);
    caps_str = gst_caps_to_string (caps);
    GST_INFO ("Using video caps: %s", caps_str);
    g_free (caps_str);
